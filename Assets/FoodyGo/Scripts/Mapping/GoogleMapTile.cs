...
namespace packt.FoodyGO.Mapping
{
    ...
    public class GoogleMapTile : MonoBehaviour
    {
        ...
        {            
            ...
            print(string.Format("Tile {0}x{1} requested with {2}", TileOffset.x, TileOffset.y, queryString)); //エラーでてる
            ...
            if (req.error != null) {
                print(string.Format("Error loading tile {0}x{1}:  exception={2}",
                    TileOffset.x, TileOffset.y, req.error));  //HTTP 403 Forbit　エラーでてる
            } else {
